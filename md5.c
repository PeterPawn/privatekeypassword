...
/* 
 * taken from uClibC sources for privatekeypassword library
...
/* compute MD5 value with a single call */

void EXPORTED md5(unsigned char * output, const unsigned char * input, unsigned int inputLen)
{
	__md5_Init(&md5_Context);
	__md5_Update(&md5_Context, input, inputLen);
	__md5_Final(output, &md5_Context);
}
